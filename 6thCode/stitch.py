...
def stitch_two_images(image1: np.ndarray, image2: np.ndarray, H: np.ndarray) -> np.ndarray:
    ...
    # H는 Global-Space Homography (Image -> Global)
    # inverse_warp 내부에서 offset을 처리하므로 H를 직접 전달
    warped2 = inverse_warp(image2, H, (canvas_height, canvas_width), 
...
def stitch_multiple_images(images: list, homographies: list) -> np.ndarray:
    ...
    for i, (image, H) in enumerate(zip(images[1:], homographies)):
        ...
        # 이미지 워핑
        # cumulative_H는 Global-Space Homography (Image -> Global)
        # inverse_warp 내부에서 offset을 처리하므로 cumulative_H를 직접 전달
        print(f"    워핑 중... (cumulative_H scale: {np.sqrt(cumulative_H[0,0]**2 + cumulative_H[0,1]**2):.2f})")
        warped = inverse_warp(image, cumulative_H, (canvas_height, canvas_width), offset)
