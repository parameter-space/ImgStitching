...
def compute_pairwise_homography(image1: np.ndarray, image2: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # 4. 특징점 매칭 (개선된 파라미터)
    matches = match_features(descriptors1, descriptors2, method='ssd', threshold=0.75)  # List[Tuple[int, int]]
    ...
    H, inlier_mask = ransac_homography(
        ...
        max_iterations=2000,  # 증가: 더 많은 반복으로 좋은 결과 찾기
        threshold=2.0,        # 감소: 더 엄격한 인라이어 기준
        min_inliers=20        # 증가: 더 많은 인라이어 필요
    )  # (3, 3) - float32, (M,) - bool
    
    # 7. Homography 검증
    is_valid, mean_error = validate_homography(H, points1.astype(np.float32), points2.astype(np.float32))
...
def main():
    ...
    # 3. Bundle Adjustment (Group Adjustment) - 누적 오차 감소
    ...
    for i in range(len(global_homographies) - 1):
        H_i = global_homographies[i]
        H_j = global_homographies[i + 1]
        # H_j = H_pairwise @ H_i 이므로
        # H_pairwise = H_j @ H_i^-1
        H_i_inv = np.linalg.inv(H_i)
        H_pairwise = H_j @ H_i_inv
        if H_pairwise[2, 2] != 0:
            H_pairwise = H_pairwise / H_pairwise[2, 2]
        pairwise_from_global.append(H_pairwise)
    
    # 5. 이미지 스티칭 (최적화된 Homography 사용)
    panorama = stitch_multiple_images(images, pairwise_from_global)  # (H_out, W_out, 3) - uint8
