...
def compute_global_homographies(pairwise_homographies: List[np.ndarray]) -> List[np.ndarray]:
    ...
    for i, H_pairwise in enumerate(pairwise_homographies):
        # 누적 변환: H_pairwise[i]는 images[i+1]을 images[i]로 변환
        # cumulative_H는 images[i]를 images[0]로 변환
        # images[i+1]을 images[0]로 변환하려면:
        # images[i+1] -> images[i] (H_pairwise) -> images[0] (cumulative_H)
        # 따라서 cumulative_H = cumulative_H @ H_pairwise (먼저 H_pairwise 적용, 그 다음 cumulative_H 적용)
